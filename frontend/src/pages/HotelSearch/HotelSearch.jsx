...
export default function HotelSearch() {
    return (
        <PageTransition>
            <HeaderOfPage className="h-[450px] md:h-[540px] pt-20 bg-hotel_search bg-cover bg-center" title="Make your travel whishlist, we’ll do the rest" subTitle="Special offers to suit your plan" />
            <main className="-translate-y-32 mb-[200px]">
                <div className="container">
                    <div>
                        <SearchBox title="Where are you flying?" btn={<Button title="Show Places" to='all' size="lg" beforeIcon={<Svg iconID="building" className="w-5 h-5" />} />}>
                            <HotelSearchForm className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-9 gap-6" />
                        </SearchBox>
                    </div>
                    ...
                    <Section className="mt-20">
                        ...
                        <GridSystem parentClaasName="grid grid-cols-4 gap-4 mt-10">
                            <div className="col-span-4 order-1 lg:order-none lg:col-span-2 row-span-2 bg-primary p-6 rounded-2xl flex flex-col justify-between">
                                ...
                                <Link to='all' className="py-2 px-4 h-12 flex-center bg-white text-slate-900 font-MontserratMedium text-sm rounded transition-all hover:bg-slate-50 mt-28 lg:mt-0">Book Hotels</Link>
                            </div>
